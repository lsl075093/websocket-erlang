...
%%--------------------------------------------------------------------
-spec(init(Args :: term()) ->
    {ok, State :: #socket_conn{}} | {ok, State :: #socket_conn{}, timeout() | hibernate} |
...
-spec(handle_call(Request :: term(), From :: {pid(), Tag :: term()},
    State :: #socket_conn{}) ->
    {reply, Reply :: term(), NewState :: #socket_conn{}} |
    {reply, Reply :: term(), NewState :: #socket_conn{}, timeout() | hibernate} |
    {noreply, NewState :: #socket_conn{}} |
    {noreply, NewState :: #socket_conn{}, timeout() | hibernate} |
    {stop, Reason :: term(), Reply :: term(), NewState :: #socket_conn{}} |
...
%% @end
%%--------------------------------------------------------------------
-spec(handle_cast(Request :: term(), State :: #socket_conn{}) ->
    {noreply, NewState :: #socket_conn{}} |
    {noreply, NewState :: #socket_conn{}, timeout() | hibernate} |
    {stop, Reason :: term(), NewState :: #socket_conn{}}).
...
%% @end
%%--------------------------------------------------------------------
-spec(handle_info(Info :: timeout() | term(), State :: #socket_conn{}) ->
    {noreply, NewState :: #socket_conn{}} |
    {noreply, NewState :: #socket_conn{}, timeout() | hibernate} |
    {stop, Reason :: term(), NewState :: #socket_conn{}}).
handle_info({tcp, Socket, Bin}, #socket_conn{socket = Socket, data = DataOld, msg_queue = Q} = State) ->
    case decode(Bin) of
        close ->
            ...
            {stop, normal, State};
        {go_end, DataAdd} ->
            Msg = <<DataOld/binary, DataAdd/bianry>>,
        ...
        {go_one, DataAdd} ->
            {noreply, State#socket_conn{data = <<DataOld/binary, DataAdd/bianry>>}};
        _ ->
            {noreply, State}
    end;
...
%% @end
%%--------------------------------------------------------------------
-spec(code_change(OldVsn :: term() | {down, term()}, State :: #socket_conn{},
    Extra :: term()) ->
    {ok, NewState :: #socket_conn{}} | {error, Reason :: term()}).
...
decode(Bin) ->
    ...
    if
        Opcode == 8 -> close;
        Opcode == 0 orelse Opcode == 1 orelse Opcode == 2  ->
            State = case Opcode =:= 0 of
                        true  -> go_on;
                        false -> go_end
                    end,
            case Payload of
                ...
                127 ->
                    StillNeed = 8*8, Add = 0;
                _ ->
                    StillNeed = 0, Add = Payload
            end,
            {State, decode1(Rest, StillNeed, Add)};
        _ ->
            nothing
    end.
decode1(Bin, 0, L) ->
